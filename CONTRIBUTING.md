# PeriForm Contributor Guide

Contributions to PeriForm, whether new features or bug fixes, are deeply appreciated and benefit the whole user community.
...
If you'd like to contribute code or documentation to PeriForm, we welcome pull requests.
...
- **Design discussion.** For new features, some discussion on the issue will take place to determine if it's something that should be included in PeriForm. For defects, discussion may happen around whether the issue is truly a defect or if the behavior is correct.
- **Pull request.** Create [a pull request](https://help.github.com/articles/using-pull-requests/) on the `develop` branch of the repository to submit changes to the code based on the information in the issue ( for each pull request a feature or bug reports needs to be filled to simply tracking changed for release). Pull requests need to pass the CI build and follow coding standards. See below for more on coding standards in use with PeriForm. Note all pull requests should include accompanying unit tests to verify the work.
...
By contributing to PeriForm, you assert that:
...
PeriForm source code uses four spaces for indents. We use [EditorConfig](https://editorconfig.org/) to ensure consistent formatting in code docs. Visual Studio has this built in since VS 2017. VS Code requires the EditorConfig extension. Many other editors also support EditorConfig.
...
- Any third-party libraries consumed by PeriForm integration must have licenses compatible with PeriForm's license.
