...
/// <summary>
/// Contains Platform Invocation (P/Invoke) declarations for interacting with the
/// Windows user32.dll.  These methods form the basis for simulating keyboard
/// and mouse input as well as registering and handling global hooks and hotkeys.
/// </summary>
internal static partial class NativeMethods
{
    ...
    /// <param name="wParam">Message-specific information.</param>
    /// <param name="lParam">Message-specific information.</param>
    ...
    /// <summary>
    /// Sends a sequence of input events (keyboard, mouse or hardware) to the
    /// system.  Each call to <see cref="SendInput"/> is atomic – either all
    /// events are injected or none are.  The calling thread must be running
    /// in a context that allows user input (e.g. not a service session).
    /// </summary>
    ...
    /// <summary>
    /// Installs an application‑defined hook procedure into a hook chain.  Low‑level
    /// keyboard and mouse hooks use this function to intercept input globally.
    /// </summary>
    /// <param name="idHook">The type of hook to install (e.g. WH_KEYBOARD_LL, WH_MOUSE_LL).</param>
    ...
    internal static partial IntPtr SetWindowsHookEx(int idHook, LowLevelProc lpfn, IntPtr hMod, uint dwThreadId);
    ...
    /// <summary>
    /// Registers a system‑wide hotkey that will generate a <c>WM_HOTKEY</c> message when pressed.
    /// </summary>
    ...
    /// <param name="fsModifiers">Modifier keys (e.g. MOD_CONTROL | MOD_ALT).</param>
